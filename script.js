...
const pixelWidth = 234;
const pixelHeight = 162;
...
function paintTopPart(image, xOffset) {
...
function paintBottomPart(image, xOffset) {
  ...
  ctx.drawImage(image,
    0, segmentSize * 5,
    image.width, segmentSize * 2,
    xOffset, Math.floor(canvasSegmentHeight * 6),
    canvasSegmentWidth * 6, Math.ceil(canvasSegmentHeight * 2));
}
...
class InteractiveGameHandler {
  render(_timePassed) {
    ...
    paintTopPart(star, 0);
    paintMiddlePart(star, 0);
    paintBottomPart(star, 0);
